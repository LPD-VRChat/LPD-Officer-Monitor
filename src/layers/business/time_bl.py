...
class TimeBL(DiscordListenerBL):
    ...
    async def member_join_leave(
        ...
        # Nothing happened to a regular member
            ...
            log.debug("member_join_leave - Regular member did not do anything interesting")
            return
        ...
        # Member has joined the LPD
            ...
            log.info(
                f"{after.display_name} ({after.username}#{after.discriminator}) has been added to the database."
            )
        ...
        # Member has left the LPD
            ...
            log.info(
                f"{after.display_name} ({after.username}#{after.discriminator}) has been removed from the LPD."
            )
