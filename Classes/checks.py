...
def is_white_shirt():
    def predicate(ctx):
        officer = ctx.bot.officer_manager.get_officer(ctx.author.id)
        if officer and officer.is_white_shirt: return True
        else: raise errors.NotForYouError("This command is only for LPD White Shirts.")
    return commands.check(predicate)

def is_admin():
    def predicate(ctx):
        officer = ctx.bot.officer_manager.get_officer(ctx.author.id)
        if officer and officer.is_admin: return True
        else: raise errors.NotForYouError("This command is only for LPD Cheifs.")
    return commands.check(predicate)

def is_admin_bot_channel():
    def predicate(ctx):
        if ctx.message.channel.id != ctx.bot.settings["admin_bot_channel"]:
            raise errors.WrongChannelError("This command only works in the admin bot channel.")
        return True
    return commands.check(predicate)
