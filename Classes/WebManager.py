    ...
    async def display_officers():
        ...
        for officer in bot.officer_manager.all_officers:
            content = f"""{content}
                        ...
                        <td>{officer.squad}</td>
                        {user = await discord.fetch_user(); f'<form action="/api/time/last_active" method="post"><input type="submit" name="officer_id" value="{officer.id}" /></form>' if bot.officer_manager.get_officer(user.id) is not None else ''}
                        </tr>"""
        content = f"""{content}
