    ...
    async def display_officers():
        ...
        for officer in bot.officer_manager.all_officers:
            content = f"""{content}
                        ...
                        <td>{officer.squad}</td>
                        {f'<form action="/api/time/last_active" method="post"><input type="submit" name="officer_id" value="{officer.id}" /></form>' if bot.officer_manager.get_officer(await discord.fetch_user().id) is not None else ''}
                        </tr>"""
        content = f"""{content}
