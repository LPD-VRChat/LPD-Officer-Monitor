...
async def on_message(message):
    ...
    # If the message was sent in the #leave-of-absence channel
...
async def save_loa(officer_id, date_start, date_end, reason):
    ...
    async with db_pool.acquire() as conn:
        ...
        await cur.execute("REPLACE INTO `LeaveTimes` (`officer_id`,`date_start`,`date_end`,`reason`) VALUES (%s, %s, %s, %s)", (officer_id, date_start, date_end, reason))
        await cur.close()
...
async def process_loa(message):
    ...
    try:
        ...
        await message.channel.send(message.author.mention + ' Please use correct formatting: 21/July/2020 - 21/August/2020: Reason.', delete_after=10)
        await message.delete()
        return


    try: int(date_start[1])
    except:
        try:
    ...
    if date_start[1] < 1 or date_start[1] > 12 or date_end[1] < 1 or date_end[1] > 12:
        ...
        await message.channel.send(message.author.mention + ' There are only 12 months in a year, Einstein.', delete_after=10)
        await message.delete()
        return

    date_start_complex = str(date_start[0]) + '/' + str(date_start[1]) + '/' + str(date_start[2])
    date_end_complex = str(date_end[0]) + '/' + str(date_end[1]) + '/' + str(date_end[2])
    date_start = datetime.datetime.strptime(date_start_complex, '%d/%m/%Y')
    date_end = datetime.datetime.strptime(date_end_complex, '%d/%m/%Y')

    if date_end > date_start + datetime.timedelta(weeks=+12) or date_end < date_start + datetime.timedelta(weeks=+4):
        # If more than 12 week LOA, inform user
        await message.channel.send(message.author.mention + ' Leaves of Absence are limited to 4-12 weeks. No longer, no shorter.', delete_after=10)
        await message.delete()
        return
    ...
    await message.channel.send(message.author.mention + ' your leave of absence from ' + str(date_start.date()) + ' to ' + str(date_end.date()) + ' has been updated. To edit your Leave of Absence, simply send another message.')
    await message.delete()
